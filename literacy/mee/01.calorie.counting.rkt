...
@desc{接受一个参数，返回@racket[#true]或@racket[#false]}
的函数称为@aoc-defterm[#:origin "Predicate Function"]{断言函数}。
在 Racket 中，断言函数的名字通常以@litchar{?}结尾。
...
这是一类特殊函数，它们通过对自身求值来将复杂的递归问题拆解成规模更小的子问题以各个击破。
...
而是@term-name{比较}@math{a}与@racket[2]的大小。
因此，在假设了@${a = 1}的前提下，
...
真程序语言起源于上世纪50年代，
但那些无外乎是对机器语言的直白翻译（用英文单词来替代@racket[01]串），
语法上也是互相借鉴。
总之，没有特别亮眼的创新，
特别是缺乏数学的美感。
...
@idea{程序编制者应该关注问题本身，而不是被淹没在琐碎的机器细节里，
而从人类思维到机器思维之间的脏活，该由程序语言来搞定}。
在 Lisp 的所有方言里，Racket 这一支最为看重数学理论根基的优雅和纯粹，
...
@note{实际上，@term-name{列表}是个容易引起混乱的名字，
...
我们只需要弄清楚@desc{做什么}而不用浪费时间去想@desc{怎么做}。
...
@tech{高阶函数}@id{index-where}的第二参数是一个@tech{断言函数}，
...
返回一个符合题意的@tech{断言函数}。
因为@id{<}至少需要两个操作数，我们只准备了一个，
另一个由@racket[index-where]在检查列表时依次提供。
