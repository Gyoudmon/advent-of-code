...
到这里，今天的谜面就已经都出来了。不妨就先开始定义主函数 @racketid[find-misplaced-items]
用以找出所有装错了的物品类型。按照前两天的思路，该函数内部应该也是一个类似@racketcommentfont{读取-查错-处理}的大循环。
...
找到放错了的物品类型都是第一步。之后无论精灵想做什么，你只需要提供给他们一份@racketcommentfont{出错物品类型清单}，
而不必每次都从头开始@racketcommentfont{读取}他们整理的物品清单。
...
@handbook-chunk[<定义函数find-misplaced-items>
                ...
                (define find-misplaced-items
                  (lambda [/dev/datin]
                    (for/list ([line (in-lines /dev/datin)])
                      <从清单行中找出那个放错了的物品类型>)))]

这次主函数代码量少了很多，但是增加了巨量的信息，需要慢慢消化。首先，回归正题，我们需要做的事是，针对每一行物品清单，
@question{从中找出放错了的那个物品类型}。
...
并将每一行转化后的字符收集起来作为一个列表返回。这段啰嗦的描述是不是很眼熟？正是前两天任务中需要你受累一键一键敲出来
的大循环。这次，在我们深刻理解了这个大循环的前提下，就不必再敲那么多次键盘了。函数 @racket[in-lines] 的作用是“按行
读出字符串，并将每一行依次保存在变量 @racketcommentfont{line} 中，供循环使用”，@racket[for/list] 的名字暴露出了
它(与 @racket[in-lines] 配合时)自带循环效果，并保证将每一轮循环的结果都搜集到列表中（而不是别的什么玩意里）去。

关于@bold{函数式编程}，今天我们只说这么多，最通俗易懂的部分。深呼吸，放松一下，做点简单的事开心一下：完成函数
...
回忆一下，当碰到不认识的汉字或英文单词时，你会怎么来认识它？但是，现在我们要重点思考的内容，不是你根据什么规则来快速查字典，
...
C++ 的哈希叫映射（map）；C 不直接提供。

总之，计算机也有查字典的权利和能力。于是，@question{找到放错了的物品类型}问题就转化成了@racketcommentfont{造字典-查字典}
问题，这个字典里只包含某个背包的一个隔间里的全部物品类型。

@handbook-chunk[<创建物品类型字典>
                (define dict (make-hasheq))
                
                (for ([ch (in-string line 0 midpos)])
                  (define n (hash-ref dict ch 0))
                  (hash-set! dict ch (add1 n)))]

这个 @racket[for] 和 @racket[in-string] 组合似乎有点眼熟。没错，算法中的循环又被它俩隐藏了。函数 @racket[in-string]
负责依次提取字符串指定范围内的字符，并分别保存在变量 @racketcommentfont{ch} 中供循环使用。每次往字典里添加条目必须要提供
两个参数，一个代表关键字（key），一个代表该关键字在字典里的内容（value）。比如，英汉字典里的每一个词条，英文单词是其关键字，
其对应的汉语翻译是其在字典里的内容。在本任务中，我们关注某个物品是否存在，那该物品的类型即是关键字；而其在字典中的内容是关于其
“存在与否”的信息。考虑到同一个背包隔间里可能存在多个相同类型的物品，因此字典里与关键字关联的内容可设定为该关键字出现的次数。
而造字典的过程，显然也涉及到查字典的步骤。
...
这次隐藏循环的是 @racket[for/first]，它的意思@racketcommentfont{循环执行一次就立即退出}，因此判断循环可以执行的条件极为关键。
在本任务中，即：一旦出现字典中存在的物品类型就不用再继续找下去了，直接返回该物品类型即可。特别的，如果输入文件有错误导致循环一次
都没有执行，那函数最终会返回 @racket[#false]。在真实的软件中，必须要考虑这种情况。

至此，我们完成了对物品清单的解析。至于为什么要同时定义 @racket[find-misplaced-items] 和 @racket[find-misplaced-item]
两个函数，后面这个看似完全多余啊，算法可以直接放在 @racket[find-misplaced-items] 循环里面吗？可以啊，不影响本次任务。但是，
还记得我们上面分析的结论吗：@racketcommentfont{针对清单中的每一行的操作}就转化为了@racketcommentfont{针对字符串列表中
的每一项的操作}。换句话说，我们可以直接调用 @racket[find-misplaced-item]，并传给它一个字符串作为参数，用于测试算法是否靠谱，
而不用每次都去准备一个文件来输入。比如，就用任务例子中的字符串好了：
...
可不可以一边创建一边检查呢？请课后思考并写程序验证你想法。【提示：同时给两个隔间分别创建一个字典，检查时两个隔间同时交叉比较。】


任务还没结束。接下来，为了定好出错物品类型的重组顺序，我们规定每种类型的物品都有一个优先级：
...
@handbook-chunk[<定义物品类型优先级函数>
                (code:comment #,($argv [type "代表物品类型的字符"]))
                (define (item-type-priority type)
                  (cond [(char<=? #\a type #\z)
                         (+ (char->integer type) (- 97) 1)]
                        [(char<=? #\A type #\Z)
                         (+ (char->integer type) (- 65) 27)]
                        [else 0]))

                (code:comment #,($argv [types "代表物品类型的字符列表"]))
                (define (priority-sum types)
                  (for/sum ([type (in-list types)]
                            #:when (char? type))
                    (item-type-priority type)))]

优先级函数没啥特别要说的，如果对其中的神秘数字 @racket[97] 和 @racket[65] 感兴趣，可以搜索关键字 @bold{ASCII 码}，
看看 @litchar{A} 和 @litchar{a} 在该编码中的具体位置。
