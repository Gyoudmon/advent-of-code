...
就不必再敲那么多次键盘了。函数 @racket[in-lines] 的作用是“按行读出字符串，并将每一行依次保存在变量 @racketcommentfont{line}
中，供循环使用”，@racket[for/list] 的名字暴露出了它(与 @racket[in-lines] 配合时)自带循环效果，并保证将每一轮循环处理得到
的结果都搜集到列表（而不是别的什么数据结构）中去。
...
这次隐藏循环的是 @racket[for/first]，它的意思@racketcommentfont{循环执行一次就立即退出}，因此确定循环是否执行的条件至关重要。
...
实际包含在最终的可执行程序里。因为这个循环过于常用，以至于我们完全没必要每次都敲一遍，把精力省下来关注@question{如何将每一行
